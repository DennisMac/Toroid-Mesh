...
public class ToroidMeshGenerator : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        CreateShape(Vector3.zero, 4f, 1f, new Vector3(4,0,0), new Vector3(0,0,4f));
        UpdateMesh();
    ...
    private void CreateShape(Vector3 center, float R, float r, Vector3 start, Vector3 finish, Plane? _planeOfArc = null )
    {
        ...
        float innerR = 1;               //inner radius
        float outerR = 2;               //ring radius(thick part of donut)

        float phi = 0.0f;
        float dp = (2 * Mathf.PI) / sl; 
        float theta = 0.0f;
        float dt = (2 * Mathf.PI) / st;
        ...
        for (int stack = 0; stack < st; stack++) //for stack = 0, st do
        {
            ...
            {
                phi = dp * slice;
                Vector3 v = new Vector3(Mathf.Cos(theta) * (outerR + Mathf.Cos(phi) * innerR), Mathf.Sin(theta) * (outerR + Mathf.Cos(phi) * innerR), Mathf.Sin(phi) * innerR);
                vertList.Add(v);
            }
        }
